...
class SIFA(nn.Module):
    ...
    def update_GAN(self, imagesa, imagesb):
        ...
        g_loss = loss_cycleA + loss_cycleB + self.backward_G(self.disB, self.fakeB) + self.backward_G(self.disA,
                                                                                                      self.fakeA)
        g_loss.backward()
        self.gen_opt.step()
        self.dec_opt.step()
    ...
    def update_seg(self, imagesa, imagesb, labelsa):
        ...
        print(self.segloss(self.pred_mask_fake_b, self.labelA), "seg yqk")
        print(self.criterionL1(self.realA, self.cycleA), "L1 yqk")
        print(self.criterionL1(self.realB, self.cycleB), "L2 yqk")
        print(0.1 * self.backward_G(self.disA, self.fakeA), "back1 yqk")
        print(0.1 * self.backward_G(self.disSeg, self.pred_mask_b), "back2 yqk")
        print(0.1 * self.backward_G(self.disA, self.fakeA, aux=True), "back3 yqk")
        seg_loss_B = self.segloss(self.pred_mask_fake_b, self.labelA) + \
                     ...
                     0.1 * self.backward_G(self.disA, self.fakeA) + 0.1 * self.backward_G(self.disSeg,
                                                                                          self.pred_mask_b) + 0.1 * self.backward_G(
            self.disA, self.fakeA, aux=True)
        print(seg_loss_B, "seg_loss_B ")
        seg_loss_B.backward()
        self.enc_opt.step()
        self.seg_opt.step()
