...
def get_filter(filt_size=3):
    ...
    b = a[:, None] * a[None, :]
    filt = torch.Tensor(b[:, None] * b[None, :])
    filt = filt / torch.sum(filt)

    return filt


def get_filter2d(filt_size=3):
    if (filt_size == 1):
        a = np.array([1., ])
    elif (filt_size == 2):
        a = np.array([1., 1.])
...
class Downsample(nn.Module):
    def __init__(self, channels, depths, pad_type='replicate', filt_size=3, stride=2, pad_off=0):
        ...
        self.pad_sizes = [int(1. * (filt_size - 1) / 2), int(np.ceil(1. * (filt_size - 1) / 2)),
                          ...
                          int(1. * (filt_size - 1) / 2), int(np.ceil(1. * (filt_size - 1) / 2))]
        # print(self.pad_sizes)  # (1, 1, 1, 1, 1, 1)
    ...
    def forward(self, inp):
        ...
        if (self.filt_size == 1):
            ...
            # print(self.pad(inp).shape, self.filt.shape, self.stride, inp.shape[1])
...
class Downsample2d(nn.Module):
    def __init__(self, channels, pad_type='replicate', filt_size=3, stride=2, pad_off=0):
        ...
        filt = get_filter2d(filt_size=self.filt_size)
        self.register_buffer('filt', filt[None, None, :, :].repeat((self.channels, 1, 1, 1)))

        self.pad = get_pad_layer2d(pad_type)(self.pad_sizes)
...
class Upsample(nn.Module):
    def __init__(self, channels, depths, pad_type='repl', filt_size=4, stride=2):
...
class MappingF(nn.Module):
    def __init__(self, in_layer=4, gpu_ids=[], nc=256, patch_num=256, dim=64, init_type='normal', init_gain=0.02):
        ...
        conv = nn.Conv3d(in_layer, dim, 3, stride=2)
...
class ResnetGeneratorV3(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6,
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            if (no_antialias):
                model_in += [nn.Conv3d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                             norm_layer(ngf * mult * 2),
                             nn.ReLU(True)]
            else:
                model_in += [nn.Conv3d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                             norm_layer(ngf * mult * 2),
                             nn.ReLU(True),
                             Downsample(ngf * mult * 2)]
        ...
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            if no_antialias_up:
                model_out += [nn.ConvTranspose3d(ngf * mult, int(ngf * mult / 2),
                                                 kernel_size=3, stride=2,
                                                 padding=1, output_padding=1,
                                                 bias=use_bias),
                              norm_layer(int(ngf * mult / 2)),
                ...
                model_out += [Upsample(ngf * mult),
                              ...
                              norm_layer(int(ngf * mult / 2)),
                              nn.ReLU(True)]
        model_out += [nn.ReflectionPad3d(3)]
        model_out += [nn.Conv3d(ngf, output_nc, kernel_size=7, padding=0)]
        model_out += [nn.Tanh()]
...
class ResnetGeneratorV4(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6,
                 padding_type='replicate', no_antialias=False, no_antialias_up=False, opt=None):
        """Construct a Resnet-based generator
        ...
        model = [nn.ReplicationPad3d((3, 3, 3, 3, 1, 1)),
                 nn.Conv3d(input_nc, ngf, kernel_size=(3, 7, 7), padding=0, bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        for _ in range(n_downsampling):  # add downsampling layers
            ...
            if (no_antialias):
                ...
                model += [nn.Conv3d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True),
                          Downsample(ngf * mult * 2, depths=11)]
        ...
        for i in range(n_upsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                ...
                model += [Upsample(ngf * mult, depths=3),
                          nn.Conv3d(ngf * mult, int(ngf * mult / 2),
                                    kernel_size=3, stride=1,
                                    padding=1,  # output_padding=1,
                                    bias=use_bias),
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if (no_antialias):
                ...
                model += [nn.Conv3d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True),
                          Downsample(ngf * mult * 2, depths=6)]
        ...
        for i in range(n_upsampling):  # add upsampling layers
            if no_antialias_up:
                ...
                model += [Upsample(ngf * mult, depths=6),
                          nn.Conv3d(ngf * mult, int(ngf * mult / 2),
                                    kernel_size=3, stride=1,
                                    padding=1,  # output_padding=1,
                                    bias=use_bias),
...
class ResnetGeneratorV5(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6,
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if (no_antialias):
                model += [
                    nn.Conv3d(ngf * mult, int(ngf * mult * 2.25), kernel_size=3, stride=2, padding=1, bias=use_bias),
                    norm_layer(int(ngf * mult * 2.25)),
                    nn.ReLU(True)]
            else:
                model += [
                    nn.Conv3d(ngf * mult, int(ngf * mult * 2.25), kernel_size=3, stride=1, padding=1, bias=use_bias),
                    norm_layer(int(ngf * mult * 2.25)),
                    nn.ReLU(True),
        ...
        for i in range(n_upsampling):  # add upsampling layers
            ...
            idx_up -= 1
            if no_antialias_up:
                model += [nn.ConvTranspose3d(int(ngf * mult * 1.125), 128,
                                             kernel_size=3, stride=2,
                                             padding=1, output_padding=1,
                                             bias=use_bias),
                          norm_layer(128),
                ...
                model += [Upsample(int(ngf * mult * 1.125)),
                          nn.Conv3d(int(ngf * mult * 1.125), 128,
                                    kernel_size=3, stride=1,
                                    padding=1,  # output_padding=1,
                                    bias=use_bias),
                          norm_layer(128),
...
class DSCBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        # conv_block += [nn.Conv3d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim), nn.ReLU(True)]
        conv_block += [nn.Conv3d(dim, dim, kernel_size=3, padding=p, groups=dim, bias=use_bias),
                       norm_layer(dim),
                       nn.Conv3d(dim, dim, kernel_size=1, padding=0, bias=use_bias),
                       norm_layer(dim),
                       nn.ReLU(True)]
        if use_dropout:
            conv_block += [nn.Dropout(0.5)]
...
class HierBlock(nn.Module):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        upper_branch = [Upsample(dim),
                        nn.Conv3d(dim, dim, kernel_size=3, stride=1, padding=1, bias=use_bias),
                        norm_layer(dim),
                        nn.ReLU(True),
                        #
        ...
        bottom_branch = [nn.Conv3d(dim, dim, kernel_size=3, stride=1, padding=1, bias=use_bias),
                         norm_layer(dim),
                         nn.ReLU(True),
                         Downsample(dim),
                         #
...
class NLayerDiscriminator3D(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        if (no_antialias):
            ...
            sequence = [nn.Conv3d(input_nc, ndf, kernel_size=(1, kw, kw), stride=1, padding=(0, padw, padw)), nn.LeakyReLU(0.2, True),
                        Downsample(ndf, depths=1)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):  # gradually increase the number of filters
            nf_mult_prev = nf_mult
            ...
            if (no_antialias):
                sequence += [
                    nn.Conv3d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=(1, kw, kw), stride=2, padding=(0, padw, padw), bias=use_bias),
                    norm_layer(ndf * nf_mult),
                    nn.LeakyReLU(0.2, True)
                ]
            else:
                sequence += [
                    nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=(1, kw, kw), stride=1, padding=(0, padw, padw), bias=use_bias),
                    norm_layer(ndf * nf_mult),
                    nn.LeakyReLU(0.2, True),
                    Downsample(ndf * nf_mult, depths=1)]
        ...
        sequence += [
            nn.Conv2d(ndf * nf_mult, 1, kernel_size=(1, kw, kw), stride=1, padding=(0, padw, padw))]  # output 1 channel prediction map
        self.model = nn.Sequential(*sequence)
        print(self.model)
    ...
    def forward(self, input):
        ...
        conv3 = nn.Conv3d(1, 3, (1, 3, 3), device='cuda:0')
...
class NLayerDiscriminator3D_bk(nn.Module):
    """Defines a PatchGAN discriminator"""
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            if (no_antialias):
                sequence += [
                    nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=(1, kw, kw), stride=2, padding=(0, padw, padw), bias=use_bias),
                    norm_layer(ndf * nf_mult),
                    nn.LeakyReLU(0.2, True)
                ]
            else:
