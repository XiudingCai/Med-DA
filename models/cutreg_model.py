...
class CUTRegModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.opt.isTrain:
            self.visual_names = ['real_A', 'fake_B', 'real_B', 'fake_HR', 'ref_HR']
                                 # 'fake_SR', 'fake_HR', 'real_HR']
        else:
        ...
        if self.isTrain:
            ...
            # self.netSR = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, 'sr', opt.normG,
            #                                not opt.no_dropout,
            ...
            self.optimizer_G = torch.optim.Adam(chain(self.netG.parameters(), self.netR.parameters()), lr=opt.lr, betas=(opt.beta1, opt.beta2))
            self.optimizer_D = torch.optim.Adam(self.netD.parameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2))
            # self.optimizer_SR = torch.optim.Adam(self.netSR.parameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2))
            # self.optimizer_R = torch.optim.Adam(chain(self.netR.parameters()), lr=1e-4, betas=(0.5, 0.999))
    ...
    def compute_R_loss(self):
        ...
        self.fake_lr = Upsample2(scale_factor=2, mode='bilinear')(self.netG(self.real_A))

        # print(self.fake_SR.detach().shape, self.real_HR.shape)
        self.fake_HR, dvf = self.netR(self.fake_lr, self.real_HR)
        self.loss_Reg = self.opt.lambda_R * self.calculate_reg_loss((self.fake_HR, dvf), self.real_HR)

        if self.opt.isTrain:
            self.fake_HR = self.fake_HR[:, :, ::2, ::2]
    ...
    def compute_G_loss(self):
        ...
        # Reg
        self.fake_lr = Upsample2(scale_factor=2, mode='bilinear')(fake)

        # print(self.fake_SR.detach().shape, self.real_HR.shape)
        self.fake_HR, dvf = self.netR(fake, self.real_B)
        self.loss_Reg = self.opt.lambda_R * self.calculate_reg_loss((self.fake_HR, dvf), self.real_B)
