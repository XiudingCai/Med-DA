...
class CUTSRModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE', 'Reg', 'SR', 'HR']

        self.visual_names = ['real_A', 'fake_B', 'real_B', 'before_R', 'after_R', 'ref_HR']
        ...
        if self.isTrain:
            self.model_names = ['G', 'F', 'D', 'R', 'SR']
        else:  # during test time, only load G
            self.model_names = ['G', 'SR']
        ...
        if self.isTrain:
            ...
            self.optimizer_R = torch.optim.Adam(chain(self.netSR.parameters(), self.netR.parameters()), lr=1e-4, betas=(0.5, 0.999))
    ...
    def optimize_parameters(self):
        ...
        # update HR
        self.optimizer_R.zero_grad()
        self.loss_HR = self.compute_SR_loss()
        self.loss_HR.backward()
        self.optimizer_R.step()

        # update SR
        # self.optimizer_SR.zero_grad()
        # self.loss_SR = self.compute_SR_loss()
    ...
    def compute_R_loss(self):
        ...
        self.fake_lr = Upsample2(scale_factor=2, mode='bilinear')(self.fake_B.detach())
        self.fake_SR = self.netSR(self.fake_lr)

        self.loss_SR = self.criterionIdt(self.fake_SR, self.fake_lr) * 10

        # self.fake_SR = self.netSR(self.fake_B.detach())
        self.before_R = self.fake_SR[:, :, ::2, ::2]

        # print(self.fake_SR.detach().shape, self.real_HR.shape)
        self.fake_HR, dvf = self.netR(self.fake_SR, self.real_HR)
        self.loss_Reg = self.opt.lambda_R * self.calculate_reg_loss((self.fake_HR, dvf), self.real_HR)

        self.after_R = self.fake_HR[:, :, ::2, ::2]
        # self.reg_cmap = plt.cm.coolwarm((self.after_R.detach().squeeze(1) - self.before_R.squeeze(1)).cpu().numpy())[..., :3][[1]].transpose(0, 3, 1, 2)

        return self.loss_SR + self.loss_Reg


    def compute_SR_loss(self):

        self.fake_RR = self.netSR(self.real_HR)

        self.loss_HR = self.criterionIdt(self.fake_RR, self.real_HR)
