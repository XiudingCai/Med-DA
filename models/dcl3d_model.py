...
class DCL3DModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.opt.isTrain:
            visual_names_A = ['real_A2', 'fake_B2']
            visual_names_B = ['real_B2', 'fake_A2']
        else:
        ...
        if opt.nce_idt and self.isTrain:
            ...
            visual_names_A.append('idt_B2')
            visual_names_B.append('idt_A2')
    ...
    def forward(self):
        ...
        if self.opt.nce_idt:
            ...
            self.idt_A2 = self.idt_A[:, [1]]
            self.idt_B2 = self.idt_B[:, [1]]

        if not self.opt.isTrain:
            self.real_A = self.real_A2
            self.real_B = self.real_B2
            self.fake_A = self.fake_A2
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        """
        # Real
        pred_real = netD(real.reshape(-1, 1, real.size(2), real.size(3)))
        loss_D_real = self.criterionGAN(pred_real, True)
        # Fake
        pred_fake = netD(fake.detach().reshape(-1, 1, fake.size(2), fake.size(3)))
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_GAN > 0.0:
            pred_fakeB = self.netD_A(fakeB.reshape(-1, 1, fakeB.size(2), fakeB.size(3)))
            pred_fakeA = self.netD_B(fakeA.reshape(-1, 1, fakeA.size(2), fakeA.size(3)))
