...
class CUT3Dv1Model(BaseModel):
    ...
    def compute_D_loss(self):
        ...
        pred_fake = self.netDI(fake.reshape(-1, 1, self.opt.crop_size, self.opt.crop_size))
        self.loss_DI_fake = self.criterionGAN(pred_fake, False).mean()
        # Real
        self.pred_real = self.netDI(self.real_B.reshape(-1, 1, self.opt.crop_size, self.opt.crop_size))
        ...
        ################## Dv ##################
        # Fake; stop backprop to the generator by detaching fake_B
        pred_fake_v = self.netDV(fake)
        # print(pred_fake_v.shape)
        self.loss_DV_fake = self.criterionGAN(pred_fake_v, False).mean()
        # Real
        self.pred_real_v = self.netDV(self.real_B)
        ...
        self.loss_D = (self.loss_DI + self.loss_DV) * 0.5
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_GAN > 0.0:
            ...
            pred_fake = self.netDV(self.fake_B)
            self.loss_G_GAN += self.criterionGAN(pred_fake, True).mean() * self.opt.lambda_GAN
        else:
            self.loss_G_GAN = 0.0
