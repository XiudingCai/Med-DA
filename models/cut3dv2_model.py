...
class CUT3Dv2Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionFC = self.calculate_FC_loss
    ...
    def data_dependent_initialize(self, data):
        ...
        self.B, self.C, self.H, self.W = self.real_A.shape
    ...
    def set_input(self, input):
        ...
        self.rand_B2 = 0.5 * self.real_B1 + 0.5 * self.real_B3
        ...
        self.rand_B = torch.cat([self.real_B1, self.rand_B2, self.real_B3], dim=1)
    ...
    def forward(self):
        ...
        # BCHW -> (2B)CHW
        self.real = torch.cat((self.real_A, self.rand_B),
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_FC > 0.0:
            self.loss_FC = self.opt.lambda_FC * self.criterionFC()
        else:
            self.loss_FC = 0.0
    ...
    def calculate_FC_loss(self):
        # preprocess self.idt_B
        # rand_B <-> ranx_B, idt_B <-> ixt_B
        self.ranx_B = torch.cat([self.fake_B1, 0.5 * self.fake_B1 + 0.5 * self.fake_B3, self.fake_B3], dim=1)
        self.ixt_B = self.netG(self.ranx_B)

        self.loss_FC = self.criterionIdt(self.ixt_B[:, 1], self.idt_B.detach()[:, 1])

        return self.loss_FC

    def calculate_semantic_loss(self, src, tgt):
        n_layers = len(self.nce_layers)
