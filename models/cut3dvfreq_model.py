...
class CUT3DvfreqModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D', 'G', 'NCE', 'freq']
    ...
    def compute_D_loss(self):
        ...
        self.loss_freq = self.cal_freq_loss(fake, real)
        self.loss_D_real = (1 + 1e-5 * self.loss_freq) * loss_D_real.mean()
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_GAN > 0.0:
            pred_fake = self.netD(fake)
            self.loss_G_GAN = (1 + self.cal_freq_loss(fake, real)) * self.criterionGAN(pred_fake, True).mean() * self.opt.lambda_GAN
        else:
            self.loss_G_GAN = 0.0
    ...
    def cal_freq_loss(self, x_fake, x_real):
        N = 179
        epsilon = 1e-8
        ...
        for t in range(x_fake.shape[0]):
            ...
            psd1D_fake.append(psd1D)
        psd1D_fake = torch.stack(psd1D_fake)
        ...
        for t in range(x_real.shape[0]):
            ...
            psd1D = azimuthalAverage(magnitude_spectrum)
            psd1D = (psd1D - torch.min(psd1D)) / (torch.max(psd1D) - torch.min(psd1D))
            psd1D_real.append(psd1D)
        psd1D_real = torch.stack(psd1D_real)

        loss_freq = torch.nn.BCELoss()(psd1D_fake, psd1D_real)
