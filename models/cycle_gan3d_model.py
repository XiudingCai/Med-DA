...
from util.image_pool import ImagePool, resize
...
class CycleGAN3DModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        visual_names_A = ['real_A_', 'fake_B_', 'rec_A_']
        visual_names_B = ['real_B_', 'fake_A_', 'rec_B_']

        if self.isTrain and self.opt.lambda_identity > 0.0:  # if identity loss is used, we also visualize idt_B=G_A(B) ad idt_A=G_A(B)
            visual_names_A.append('idt_B_')
            visual_names_B.append('idt_A_')
        ...
        if self.isTrain:  # define discriminators
            self.netD_A = networks.define_D(opt.output_nc, opt.ndf, opt.netD, opt.n_layers_D, opt.normD,
                                            opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD, opt.n_layers_D, opt.normD,
                                            opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def set_input(self, input):
        ...
        self.real_A_ = resize(self.real_A[:, :, 36, :, :])
        self.real_B_ = resize(self.real_B[:, :, 36, :, :])
    ...
    def forward(self):
        ...
        print(f"fake_min: {self.fake_B.min().item():.4f} ({self.real_A.min().item():.4f}), "
              f"fake_max: {self.fake_B.max().item():.4f} ({self.real_A.max().item():.4f})")
        # to display by visdom
        self.fake_B_ = resize(self.fake_B[:, :, 36, :, :])
        self.rec_A_ = resize(self.rec_A[:, :, 36, :, :])
        self.fake_A_ = resize(self.fake_A[:, :, 36, :, :])
        self.rec_B_ = resize(self.rec_B[:, :, 36, :, :])
    ...
    def test(self):
        ...
        """
        model = self.netG_A if self.opt.direction == 'AtoB' else self.netG_B
        ...
        with torch.no_grad():
            fakes = model_inferer(self.real_A)
            print(fakes.min(), fakes.max())
            real_list = decollate_batch(self.real_A)
            fake_list = decollate_batch(fakes)
        ...
        for i in range(len(img_paths)):
            ...
            real = real_list[i][0]  # * 1000
            fake = fake_list[i][0]  # * 1000
            ...
            sitk.WriteImage(nii, os.path.join(save_dir, 'fake', name))

            nii = sitk.GetImageFromArray(real.cpu().numpy().astype('int'))
            nii.SetOrigin([x.item() for x in self.image_meta['origin']])
            nii.SetSpacing([x.item() for x in self.image_meta['spacing']])
            nii.SetDirection([x.item() for x in self.image_meta['direction']])
            sitk.WriteImage(nii, os.path.join(save_dir, 'real', name))
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            # to show
            self.idt_A_ = resize(self.idt_A[:, :, 36, :, :])
            self.idt_B_ = resize(self.idt_B[:, :, 36, :, :])
        else:
            self.loss_idt_A = 0
            self.loss_idt_B = 0
