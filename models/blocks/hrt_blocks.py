...
class HRTransBlock(nn.Module):
    def __init__(
        ...
        """
        super(HRTransBlock, self).__init__()
    ...
    def forward(self, x):
        if self.num_branches == 1:
            if not self.skip_connec:
                return self.branches[0](x)
            else:
                return self.branches[0](x) + x
...
class MlpDWBN(nn.Module):
    def __init__(
        ...
        self.act1 = act_layer()
        self.norm1 = nn.InstanceNorm2d(hidden_features)
        ...
        self.norm2 = nn.InstanceNorm2d(hidden_features)
        self.fc2 = nn.Conv2d(hidden_features, out_features, kernel_size=1)
        self.act3 = act_layer()
        self.norm3 = nn.InstanceNorm2d(out_features)
...
if __name__ == '__main__':
    ...
    block_hrt = HRTransBlock(num_branches,
                             ...
                             drop_path=drop_path)
    summary(block_hrt.cuda(), (256, 64, 64))
