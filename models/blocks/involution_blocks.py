...
class Involution2d(nn.Module):
    ...
    def __init__(self,
        ...
        :param sigma_mapping: (nn.Module) Non-linear mapping as introduced in the paper. If none BN (or IN) + ReLU is utilized
        ...
        self.sigma_mapping = sigma_mapping if sigma_mapping is not None else nn.Sequential(
            nn.InstanceNorm2d(num_features=self.out_channels // self.reduce_ratio, momentum=0.3), nn.ReLU())
...
class Involution3d(nn.Module):
    ...
    def __init__(self,
        ...
        # Init modules
        self.sigma_mapping = sigma_mapping if sigma_mapping is not None else nn.Sequential(
            nn.InstanceNorm3d(num_features=self.out_channels // self.reduce_ratio, momentum=0.3), nn.ReLU())
