...
class _ConvNormActiBlockWithRAMCost(ConvNormActiBlock):
    """The class wraps monai layers with ram estimation. The ram_cost = total_ram/output_size is estimated.
    Here is the estimation:
     feature_size = output_size/out_channel
     total_ram = ram_cost * output_size
...
class _ASConvNormActiBlockWithRAMCost(ASConvNormActiBlock):
    """The class wraps monai layers with ram estimation. The ram_cost = total_ram/output_size is estimated.
    Here is the estimation:
     feature_size = output_size/out_channel
     total_ram = ram_cost * output_size
...
class _DSConvNormActiBlockWithRAMCost(DSConvNormActiBlock):
    """The class wraps monai layers with ram estimation. The ram_cost = total_ram/output_size is estimated.
    Here is the estimation:
     feature_size = output_size/out_channel
     total_ram = ram_cost * output_size
    ...
    def __init__(self, in_channel: int, out_channel: int, kernel_size: int, padding: int, spatial_dims: int = 3,
                 expand_ratio: int = 1):
        super().__init__(in_channel, out_channel, kernel_size, padding, spatial_dims, expand_ratio)
        self.ram_cost = 2 + 2 * in_channel / out_channel


class _P2DActiConvNormBlockWithRAMCost(P2DActiConvNormBlock):
    def __init__(self, in_channel: int, out_channel: int, kernel_size: int, padding: int, p2dmode: int = 0):
        super().__init__(in_channel, out_channel, kernel_size, padding, p2dmode)
...
class _ConvNormActiBlockWithCosts(ConvNormActiBlock):
    """The class wraps monai layers with ram estimation. The ram_cost = total_ram/output_size is estimated.
    Here is the estimation:
     feature_size = output_size/out_channel
     total_ram = ram_cost * output_size
    ...
    def __init__(self, in_channel: int, out_channel: int, kernel_size: int, padding: int, spatial_dims: int = 3):
        ...
        self.ram_cost = 1 + in_channel / out_channel * 2
        # FLOPs(Conv) = (2 * K^2 * C_i) * HW * C_o
        self.flops_cost = (2 * kernel_size ** 2 * in_channel) * out_channel


class _ASConvNormActiBlockWithCosts(ConvNormActiBlock):
    """The class wraps monai layers with ram estimation. The ram_cost = total_ram/output_size is estimated.
    Here is the estimation:
     feature_size = output_size/out_channel
     total_ram = ram_cost * output_size
...
class _FactorizedIncreaseBlockWithCosts(FactorizedIncreaseBlock):
    def __init__(self, in_channel: int, out_channel: int, spatial_dims: int = 3):
        super().__init__(in_channel, out_channel, spatial_dims)
        # s0 is upsampled 2x from s1, representing feature sizes at two resolutions.
        # 2 * in_channel * s0 (upsample + activation) + 2 * out_channel * s0 (conv + normalization)
...
class _FactorizedReduceBlockWithCosts(FactorizedReduceBlock):
    def __init__(self, in_channel: int, out_channel: int, spatial_dims: int = 3):
        super().__init__(in_channel, out_channel, spatial_dims)
        # s0 is upsampled 2x from s1, representing feature sizes at two resolutions.
        # in_channel * s0 (activation) + 3 * out_channel * s1 (convolution, concatenation, normalization)
...
class _AntialiasUpBlockWithCosts(AntialiasUpBlock):
    def __init__(self, in_channel: int, out_channel: int, spatial_dims: int = 3):
        super().__init__(in_channel, out_channel, spatial_dims)
        # s0 is upsampled 2x from s1, representing feature sizes at two resolutions.
        # 2 * in_channel * s0 (upsample + activation) + 2 * out_channel * s0 (conv + normalization)
...
class _AntialiasDownBlockWithCosts(AntialiasDownBlock):
    def __init__(self, in_channel: int, out_channel: int, spatial_dims: int = 3):
        super().__init__(in_channel, out_channel, spatial_dims)
        # s0 is upsampled 2x from s1, representing feature sizes at two resolutions.
        # in_channel * s0 (activation) + 3 * out_channel * s1 (convolution, concatenation, normalization)
...
class Cell(CellInterface):
    ...
    #     "conv_1x3": lambda c: _P2DActiConvNormBlockWithRAMCost(c, c, 3, padding=1, p2dmode=0),
    #     "conv_3x1": lambda c: _P2DActiConvNormBlockWithRAMCost(c, c, 3, padding=1, p2dmode=1),
    # }
    # # search space trial 2
    OPS2D = {
        # "skip_connect": lambda _c: _IdentityWithRAMCost(),
        "conv_3x3": lambda c: _ConvNormActiBlockWithRAMCost(c, c, 3, padding=1, spatial_dims=2),
        "asconv_3x3": lambda c: _ASConvNormActiBlockWithRAMCost(c, c, 3, padding=2, spatial_dims=2),
        "dsconv_3x3": lambda c: _DSConvNormActiBlockWithRAMCost(c, c, 3, padding=1, spatial_dims=2, expand_ratio=1),
    }
...
class DiNTS(nn.Module):
    ...
    def __init__(
        ...
        self.cell_tree = dints_space.cell_tree
        self.arch_code2in = dints_space.arch_code2in
        self.arch_code2out = dints_space.arch_code2out
...
class TopologyConstruction(nn.Module):
    ...
    def __init__(
        ...
        self.filter_nums = [int(n_feat * channel_mul) for n_feat in (64, 128, 256, 512, 512)]
        # self.filter_nums = [int(n_feat * channel_mul) for n_feat in (48, 96, 192, 256, 384)]
...
def main():
    ...
    x = torch.randn(1, in_nc, 32, 128, 128).cuda()
    y = model(x)
    print(y.shape)
...
def main_2d():
    ...
    x = torch.randn(1, in_nc, 256, 256).cuda()
    y = model(x)
    print(y.shape)
