...
class ShuffleBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel, stride, supernet=True):
        ...
        self.cb_main = nn.Sequential(
            ...
            nn.InstanceNorm2d(self.mid_channels, affine=self.affine),
            ...
            nn.InstanceNorm2d(self.out_channels, affine=self.affine),
            nn.ReLU(inplace=True)
        )
        if stride == 2:
            self.cb_proj = nn.Sequential(
                ...
                nn.InstanceNorm2d(self.in_channels, affine=self.affine),
                # pw
