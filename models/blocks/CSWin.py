...
class CSWinTGenerator(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2, 2, 6, 2],
        ...
        self.merge5 = Merge_Block(curr_dim, curr_dim * 2)
        curr_dim = curr_dim * 2
        ...
        self.merge6 = Merge_Block(curr_dim, curr_dim * 2)
        curr_dim = curr_dim * 2
        ...
        self.stage8_conv_embed = nn.Sequential(
            nn.Conv2d(in_chans, embed_dim, kernel_size=7, stride=4, padding=2),
            Rearrange('b c h w -> b (h w) c', h=img_size // 4, w=img_size // 4),
            nn.LayerNorm(embed_dim)
        )
    ...
    def forward_features(self, x):
        ...
        x = self.stage1_conv_embed(x)
        print('stage1_conv_embed', x.shape)  # torch.Size([1, 4096, 96])
        for blk in self.stage1:
            if self.use_chk:
                x = checkpoint.checkpoint(blk, x)
            else:
        ...
        for pre, blocks in zip([self.merge1, self.merge2, self.merge3],
            ...
            for blk in blocks:
                if self.use_chk:
                    ...
                    x = blk(x)
        # Decoding
        for pre, blocks in zip([self.merge4, self.merge5, self.merge6],
                               [self.stage5, self.stage6, self.stage7]):
            x = pre(x)
            for blk in blocks:
                if self.use_chk:
                    x = checkpoint.checkpoint(blk, x)
...
def main():
    ...
    model = CSWinTGenerator(img_size=256, patch_size=16, in_chans=3, embed_dim=96, depth=[2, 2, 2, 2],
                            split_size=[2, 4, 8],
                            num_heads=[4 for _ in range(9)], mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0.,
                            attn_drop_rate=0., drop_path_rate=0.,
                            hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False).cuda()
    y = model(x)
    print(y.shape)
