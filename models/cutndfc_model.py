...
class CUTNDFCModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D', 'G', 'NCE', 'FC']

        if self.isTrain:
            self.visual_names = ['real_A2', 'fake_B2',
                                 'real_A3', 'fake_B3',
                                 'real_A4', 'fake_B4']
        else:
        ...
        opt.input_nc = 1 * self.opt.num_K + 1
        opt.output_nc = 1 * self.opt.num_K + 1
    ...
    def set_input(self, input):
        ...
        # print(self.real_A.shape, self.real_B.shape)

        self.real_A123 = self.real_A[:, 0:3]
        self.real_A234 = self.real_A[:, 1:4]
        self.real_A345 = self.real_A[:, 2:5]
        self.real_B123 = self.real_B[:, 0:3]
        self.real_B234 = self.real_B[:, 1:4]
        self.real_B345 = self.real_B[:, 2:5]

        self.real_A2 = self.real_A[:, [1]]
        self.real_A3 = self.real_A[:, [2]]
        self.real_A4 = self.real_A[:, [3]]
        self.real_B2 = self.real_B[:, [1]]
        self.real_B3 = self.real_B[:, [2]]
        self.real_B4 = self.real_B[:, [3]]

        self.real_A = torch.cat([self.real_A123, self.real_A234, self.real_A345], dim=0)
        self.real_B = torch.cat([self.real_B123, self.real_B234, self.real_B345], dim=0)
    ...
    def forward(self):
        ...
        self.fake_B123 = self.fake_B[0 * self.opt.batch_size:1 * self.opt.batch_size]
        self.fake_B234 = self.fake_B[1 * self.opt.batch_size:2 * self.opt.batch_size]
        self.fake_B345 = self.fake_B[2 * self.opt.batch_size:3 * self.opt.batch_size]

        self.fake_B2 = self.fake_B123[:, [1]]
        self.fake_B3 = self.fake_B234[:, [1]]
        self.fake_B4 = self.fake_B345[:, [1]]

        # print(self.fake_B123.shape, self.fake_B2.shape)  # torch.Size([2, 3, 256, 256]) torch.Size([2, 1, 256, 256])

        if not self.isTrain:
            self.real_A = self.real_A3
            self.real_B = self.real_B3
            self.fake_B = self.fake_B3
    ...
    def compute_G_loss(self):
        ...
        # frame-consistency loss
        if self.opt.lambda_FC > 0.0:
            self.loss_FC1 = self.criterionFC(self.fake_B345[:, [0]], self.fake_B234.detach()[:, [1]])
            self.loss_FC3 = self.criterionFC(self.fake_B123[:, [2]], self.fake_B234.detach()[:, [1]])
            self.loss_FC2 = self.criterionFC(self.fake_B234[:, [0]], self.fake_B123.detach()[:, [1]])
            self.loss_FC4 = self.criterionFC(self.fake_B234[:, [2]], self.fake_B345.detach()[:, [1]])
            self.loss_FC = self.opt.lambda_FC * (self.loss_FC1 + self.loss_FC2 + self.loss_FC3 + self.loss_FC4)
        else:
            self.loss_FC = 0.0
