...
class CUT3Dv4Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D', 'G', 'NCE', 'StyleNCE']

        if self.isTrain:
            self.visual_names = ['real_A1', 'fake_B1', 'real_R1', 'real_S1',
                                 'real_A2', 'fake_B2', 'real_R2', 'real_S2',
                                 'real_A3', 'fake_B3', 'real_R3', 'real_S3']
        else:
        ...
        self.netS = networks.define_F(opt.input_nc, opt.netF, opt.normG, not opt.no_dropout, opt.init_type,
                                      opt.init_gain, opt.no_antialias, self.gpu_ids, opt)
    ...
    def data_dependent_initialize(self, data):
        ...
        if self.opt.isTrain:
            ...
            if self.opt.lambda_NCE > 0.0:
                ...
                self.optimizer_S = torch.optim.Adam(self.netS.parameters(), lr=self.opt.lr,
                                                    betas=(self.opt.beta1, self.opt.beta2))
    ...
    def set_input(self, input):
        ...
        if self.opt.isTrain:
            ...
            self.real_S1 = input['S_pre'].to(self.device)
            self.real_S2 = input['S'].to(self.device)
            self.real_S3 = input['S_post'].to(self.device)

            self.real_S = torch.cat([self.real_S1, self.real_S2, self.real_S3], dim=1)
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_NCE > 0.0:
            ...
            self.loss_StyleNCE = self.calculate_StyleNCE_loss(self.fake_B, self.real_S, self.real_B)
        else:
        ...
        self.loss_G = self.loss_G_GAN + (loss_NCE_both + self.loss_StyleNCE) * 0.5
        return self.loss_G
    ...
    def calculate_StyleNCE_loss(self, q, k, v):
        n_layers = len(self.nce_layers)
        ...
        # torch.Size([2, 256, 64, 64])
        # query
        feat_q = self.netG(k, self.nce_layers, encode_only=True)
        feat_k = self.netG(q, self.nce_layers, encode_only=True)
        feat_v = self.netG(v, self.nce_layers, encode_only=True)
        ...
        feat_k_pool, sample_ids = self.netS(feat_k, num_patches, None)
        # [torch.Size([512, 256]), ...]
        feat_q_pool, _ = self.netS(feat_q, num_patches, sample_ids)
        feat_v_pool, _ = self.netS(feat_v, num_patches, sample_ids)
