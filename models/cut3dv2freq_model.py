...
class CUT3Dv2freqModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'G', 'D', 'DI', 'DV', 'NCE']

        if self.isTrain:
            self.visual_names = ['real_A1', 'fake_B1', 'real_R1',
                                 'real_A2', 'fake_B2', 'real_R2',
                                 'real_A3', 'fake_B3', 'real_R3',
                                 'real_A2_fft', 'fake_B2_fft', 'real_R2_fft']
        else:
        ...
        if self.isTrain:
            opt.output_nc = 1
            self.netDI = networks.define_D(opt.output_nc, opt.ndf, opt.netD, opt.n_layers_D, opt.normD, opt.init_type,
                                           opt.init_gain, opt.no_antialias, self.gpu_ids, opt)
            self.netDV = networks.define_D(opt.output_nc, opt.ndf, opt.netD, opt.n_layers_D, opt.normD, opt.init_type,
                                           opt.init_gain, opt.no_antialias, self.gpu_ids, opt)
            ...
            self.criterionFC = torch.nn.L1Loss().to(self.device)

            # self.criterionReg = vox_morph_loss
            self.criterionReg = self.calculate_reg_loss

            # from airlab.loss.pairwise import NCC
            # self.criterionReg = NCC()

            self.optimizer_G = torch.optim.Adam(self.netG.parameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2))
            self.optimizer_DI = torch.optim.Adam(self.netDI.parameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2))
            self.optimizer_DV = torch.optim.Adam(self.netDV.parameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2))
    ...
    def set_input(self, input):
        ...
        """
        if self.opt.isTrain:
            input, self.cur_epoch = input
        AtoB = self.opt.direction == 'AtoB'
        # self.real_A = input['A' if AtoB else 'B'].to(self.device)
        # self.real_B = input['B' if AtoB else 'A'].to(self.device)

        self.real_A1 = input['A_pre'].to(self.device)
        self.real_A2 = input['A'].to(self.device)
        self.real_A3 = input['A_post'].to(self.device)
        self.real_B1 = input['B_pre'].to(self.device)
        self.real_B2 = input['B'].to(self.device)
        self.real_B3 = input['B_post'].to(self.device)
    ...
    def forward(self):
        ...
        if self.isTrain:
            self.fake_B1 = self.fake_B[:, [0]]
            self.fake_B2 = self.fake_B[:, [1]]
            self.fake_B3 = self.fake_B[:, [2]]
        else:
    ...
    def compute_D_loss(self):
        ...
        pred_fake = self.netDI(fake)
        self.loss_DI_fake = self.criterionGAN(pred_fake, False).mean()
        # Real
        self.pred_real = self.netDI(real)
        loss_DI_real = self.criterionGAN(self.pred_real, True)
        self.loss_DI_real = loss_DI_real.mean()

        # combine loss and calculate gradients
        self.loss_DI = (self.loss_DI_fake + self.loss_DI_real) * 0.5

        ################## Dv ##################
        fake_fft = self.convert_fft(fake)
        real_fft = self.convert_fft(real)
        # Fake; stop backprop to the generator by detaching fake_B
        pred_fake_v = self.netDV(fake_fft)
        self.loss_DV_fake = self.criterionGAN(pred_fake_v, False).mean()
        # Real
        self.pred_real_v = self.netDV(real_fft)
        loss_DV_real = self.criterionGAN(self.pred_real_v, True)
        self.loss_DV_real = loss_DV_real.mean()

        self.real_A2_fft = real_fft
        self.fake_B2_fft = fake_fft
        self.real_R2_fft = self.convert_fft(self.real_R2)

        # combine loss and calculate gradients
        self.loss_DV = (self.loss_DV_fake + self.loss_DV_real * 1e-2) * 0.5
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_GAN > 0.0:
            ...
            pred_fake = self.netDV(fake_fft)
            self.loss_G_GAN += self.criterionGAN(pred_fake, True).mean() * self.opt.lambda_GAN * 1e-2
        else:
            self.loss_G_GAN = 0.0
    ...
    def calculate_reg_loss(self, src, tgt, lambd=0.01):
        # prepare image loss
        loss_image = vxm.losses.NCC().loss(tgt, src[0])
        # image_loss = vxm.losses.MSE().loss(src, tgt)

        # prepare deformation loss
        int_downsize = 2
        loss_deform = lambd * vxm.losses.Grad('l2', loss_mult=int_downsize).loss(tgt, src[1])

        fake_B2 = self.fake_B.detach()
        loss_coreg = 10 * self.criterionFC(src[0], fake_B2)

        print(loss_image.item(), loss_deform.item(), loss_coreg.item())

        return loss_image + loss_deform + loss_coreg

    def convert_fft(self, img, mode=0):
        fft_img = torch.fft.fftn(img)
        fft_img = torch.log(torch.abs(fft_img) + 1e-3)

        # fft_min = torch.percentile(fft_img, 5)
        # fft_max = torch.percentile(fft_img, 95)
        tmp = fft_img.reshape(fft_img.size(0), -1)
        sorted, indices = torch.sort(tmp, dim=1)
        # torch.Size([6]) -> torch.Size([6, 1, 1, 1]), for broadcasting
        fft_min = sorted[:, int(0.05 * tmp.size(1))].view(-1, 1, 1, 1)
        fft_max = sorted[:, int(0.95 * tmp.size(1))].view(-1, 1, 1, 1)
