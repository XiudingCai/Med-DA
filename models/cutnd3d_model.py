...
class CUTND3DModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.real_A = self.real_A.unsqueeze(1)
    ...
    def forward(self):
        ...
        from models.networks3D import NLayerDiscriminator

        netD = NLayerDiscriminator(1).cuda()
        print('>>>>>>>', netD(self.fake_B.detach().squeeze().unsqueeze(1)).shape)
        print('oook')
    ...
    def compute_D_loss(self):
        ...
        print('DDDDDDDone!', pred_fake.shape)
        # from torch import nn
        # loss = nn.L1Loss()
        # loss(pred_fake, torch.randn(pred_fake.shape, device='cuda:0'))
        # print('tttttttttttone!')
        # print(pred_fake)
        self.loss_D_fake = self.criterionGAN(pred_fake, False).mean()
        os.environ['CUDA_LAUNCH_BLOCKING'] = '1'

        # Real
        print('>>>> ', self.real_B.shape, fake.shape)
    ...
    def compute_G_loss(self):
        ...
        fake = self.fake_B.squeeze(1).reshape(-1, 1, self.opt.crop_size, self.opt.crop_size)
