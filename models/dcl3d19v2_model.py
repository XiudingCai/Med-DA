...
class DCL3D19V2Model(BaseModel):
    ...
    def compute_G_loss(self):
        ...
        # segmentation loss
        self.loss_Seg_A = self.criterionSEG(torch.cat([self.real_A_SEG, self.fake_B_SEG], dim=0),
                                          torch.cat([self.real_A_gt, self.real_A_gt], dim=0)) * 2
        self.loss_Seg_B = self.criterionSEG(torch.cat([self.real_B_SEG, self.fake_A_SEG], dim=0),
                                          torch.cat([self.real_B_gt, self.real_B_gt], dim=0)) * 2
        self.loss_Seg = self.loss_Seg_A + self.loss_Seg_B
