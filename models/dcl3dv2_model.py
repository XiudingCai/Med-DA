...
class DCL3DV2Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'NCE1', 'D_B', 'G_B', 'NCE2', 'G', 'Seg', 'Reg', 'G_X', 'D_X']
        visual_names_A = ['real_A', 'fake_B', 'real_A_SEG', 'fake_REG_B', 'real_RB']
        visual_names_B = ['real_B', 'fake_A', 'real_B_SEG', 'fake_REG_A', 'real_RA']
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'F1', 'D_A', 'G_B', 'F2', 'D_B', 'S', 'D_X', 'R_A', 'R_B']
        ...
        self.netS = smp.Unet(encoder_name='resnet18', encoder_weights='imagenet',
                             in_channels=opt.input_nc * 2, classes=1).cuda()
        ...
        if self.isTrain:
            ...
            self.netD_B = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.normD, opt.init_type, opt.init_gain, opt.no_antialias,
                                            self.gpu_ids, opt)
            self.netD_X = networks.define_D(opt.output_nc * 2 + 1, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.normD, opt.init_type, opt.init_gain, opt.no_antialias,
                                            self.gpu_ids, opt)
            ...
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters(),
                                                ...
                                                lr=opt.lr, betas=(opt.beta1, opt.beta2))
            self.optimizer_S = torch.optim.Adam(self.netS.parameters(),
                                                lr=opt.lr, betas=(opt.beta1, opt.beta2))
    ...
    def forward(self):
        ...
        # segmentation
        self.real_A_SEG = self.netS(torch.cat([self.idt_B, self.fake_B], dim=1))
        self.real_B_SEG = self.netS(torch.cat([self.fake_A, self.idt_A], dim=1))

        self.real_X = torch.cat([self.fake_A, self.idt_A, self.real_B_SEG], dim=1).detach()
        self.fake_X = torch.cat([self.idt_B, self.fake_B, self.real_A_SEG], dim=1)
    ...
    def backward_D_X(self):
        ...
        self.loss_D_X = self.backward_D_basic(self.netD_X, self.real_X, fake_X) * self.opt.lambda_GAN
    ...
    def compute_G_loss(self):
        ...
        # segmentation loss
        self.loss_Seg = self.criterionSEG(self.real_B_SEG, self.real_B_gt) * 10
        self.loss_G_X = self.criterionGAN(self.netD_X(self.fake_X), True).mean() * self.opt.lambda_GAN

        self.loss_G = (self.loss_G_A + self.loss_G_B + self.loss_G_X) * 0.3 + loss_NCE_both + self.loss_Seg + self.loss_Reg
    ...
    def val_metrics(self, epoch, train_dataloader, val_dataloader):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            pred = torch.sigmoid(self.netS(torch.cat([fake_A, self.real_B], dim=1)))
        ...
        for i, data in enumerate(val_dataloader):
            ...
            pred = torch.sigmoid(self.netS(torch.cat([self.real_A, fake_B], dim=1)))
            pred = torch.argmax(torch.cat([1 - pred, pred], dim=1), dim=1, keepdim=True)
            # dice
