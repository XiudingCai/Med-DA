...
class VoxelMorph(nn.Module):
    ...
    def __init__(self, input_dims, is_2d=False, use_gpu=True):
        ...
        if is_2d:
            self.voxelmorph = VoxelMorph2d(input_dims[0] * 2, use_gpu)
        else:
            self.voxelmorph = VoxelMorph3d(input_dims[0] * 2, use_gpu)
    ...
    def forward(self, moving, fixed):
        return self.voxelmorph(moving, fixed)

    def calculate_loss(self, y, ytrue, n=9, lamda=0.01, is_training=True):
        loss = vox_morph_loss(y, ytrue, n, lamda)
        return loss
    ...
    def get_test_loss(self, batch_moving, batch_fixed, n=9, lamda=0.01):
        with torch.set_grad_enabled(False):
            registered_image = self.voxelmorph(batch_moving, batch_fixed)
            registered_image = registered_image.cuda()
            val_loss = vox_morph_loss(
...
class VoxelMorph3d(nn.Module):
    def __init__(self, in_channels=1, use_gpu=False):
        ...
        self.unet = UNet3D(in_channels, 3)
        self.spatial_transform = SpatialTransformation(use_gpu)
        if use_gpu:
            self.unet = self.unet.cuda()
            self.spatial_transform = self.spatial_transform.cuda()
