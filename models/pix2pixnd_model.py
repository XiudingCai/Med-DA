...
class Pix2PixNDModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--num_K', type=int, default=0, help='2K + 1 frames')
    ...
    def backward_D(self):
        ...
        fake_AB = torch.cat((self.real_A.reshape(-1, 1, self.opt.crop_size, self.opt.crop_size),
                             self.fake_B.reshape(-1, 1, self.opt.crop_size, self.opt.crop_size)), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        pred_fake = self.netD(fake_AB.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake, False)
        # Real
        real_AB = torch.cat((self.real_A.reshape(-1, 1, self.opt.crop_size, self.opt.crop_size),
                             self.real_B.reshape(-1, 1, self.opt.crop_size, self.opt.crop_size)), 1)
    ...
    def backward_G(self):
        ...
        fake_AB = torch.cat((self.real_A.reshape(-1, 1, self.opt.crop_size, self.opt.crop_size),
                             self.fake_B.reshape(-1, 1, self.opt.crop_size, self.opt.crop_size)), 1)
