...
class UnalignedSeqDataset(BaseDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        if index == 0:
            A_pre_path = A_path
            A_post_path = self.A_paths[(index + 1) % self.A_size]
        elif index == self.A_size - 1:
            A_pre_path = self.A_paths[(index - 1) % self.A_size]
            A_post_path = A_path
        else:
            A_pre_path = self.A_paths[(index - 1) % self.A_size]
            A_post_path = self.A_paths[(index + 1) % self.A_size]
        ...
        if index == 0:
            B_pre_path = B_path
            B_post_path = self.B_paths[(index + 1) % self.B_size]
        elif index == self.B_size - 1:
            B_pre_path = self.B_paths[(index - 1) % self.B_size]
            B_post_path = B_path
        else:
            B_pre_path = self.B_paths[(index - 1) % self.B_size]
            B_post_path = self.B_paths[(index + 1) % self.B_size]
...
#        print('current_epoch', self.current_epoch)
        ...
        A = torch.cat([A_pre, A, A_post], dim=0)
        B = transform(B_img)
        B_pre = transform(B_pre_img)
        B_post = transform(B_post_img)
        B = torch.cat([B_pre, B, B_post], dim=0)
