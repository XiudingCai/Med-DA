...
class Unaligned4CT2MRDataset(BaseDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        dx = A_path.rfind('(')
        prefix, idxpng = A_path[:dx], A_path[dx+1:]
        idx, png = idxpng.split(').')

        A_pre_path = f"{prefix}({eval(idx) - 1}).{png}"
        A_post_path = f"{prefix}({eval(idx) + 1}).{png}"
        ...
        prefix, idxpng = B_path[:dy], B_path[dy+1:]
        idx, png = idxpng.split(').')

        B_pre_path = f"{prefix}({str(int(idx) - 1)}).{png}"
        B_post_path = f"{prefix}({str(int(idx) + 1)}).{png}"
        ...
        print(A_pre_path, A_path, A_post_path)
        ...
        A_post_img = Image.open(A_post_path)
        ...
        return {'A': A, 'B': B, 'R': R,
                'A_pre': A_pre, 'B_pre': B_pre, 'R_pre': R_pre,
                'A_post': A_post, 'B_post': B_post, 'R_post': R_post,
                'A_paths': A_path, 'B_paths': B_path, 'R_paths': R_path,
                'A_pre_paths': A_pre_path, 'B_pre_paths': B_pre_path, 'R_pre_paths': R_pre_path,
                'A_post_paths': A_post_path, 'B_post_paths': B_post_path, 'R_post_paths': R_post_path}
