...
class UnalignedSlicesDataset(BaseDataset):
    """
...
#         is_finetuning = self.opt.isTrain and self.current_epoch > self.opt.n_epochs
        ...
        A = torch.cat([torch.cat([A[[0]] for _ in range(K)], dim=0), A,
                       torch.cat([A[[-1]] for _ in range(K)], dim=0)], dim=0)[idxA: idxA + 2 * K + 1]
        B = torch.cat([torch.cat([B[[0]] for _ in range(K)], dim=0), B,
                       torch.cat([B[[-1]] for _ in range(K)], dim=0)], dim=0)[idxB: idxB + 2 * K + 1]
        # print(A.shape)
